...
Heads:
    category (7-way), sub-category (28-way), priority (1-5 regression).
...
def load_taxonomy(path="config/taxonomy.json"):
    ...
    subcategories, subcat_to_category = [], {}
...
class GrieveAIClassifier(nn.Module):
    ...
    @staticmethod
    def mask_subcategory_logits(subcategory_logits, predicted_category_idx, taxonomy):
    ...
    def trainable_parameters(self):
        return [(n, p.numel()) for n, p in self.named_parameters() if p.requires_grad]


class MuRILInference:
    """Loads a compact PEFT adapter + classifier heads for Flask inference."""
    def __init__(self, checkpoint_dir, taxonomy_path="config/taxonomy.json", device=None):
        self.checkpoint_dir = Path(checkpoint_dir)
        self.taxonomy = load_taxonomy(taxonomy_path)
        self.device = torch.device(device or ("cuda" if torch.cuda.is_available() else "cpu"))
        self.tokenizer = AutoTokenizer.from_pretrained(self.checkpoint_dir / "tokenizer")
        base = AutoModel.from_pretrained(MURIL_CHECKPOINT)
        self.encoder = PeftModel.from_pretrained(base, self.checkpoint_dir / "adapter")
        hidden_size = base.config.hidden_size
        self.category_head = nn.Linear(hidden_size, len(self.taxonomy["categories"]))
        self.subcategory_head = nn.Linear(hidden_size, len(self.taxonomy["subcategories"]))
        self.priority_head = nn.Linear(hidden_size, 1)
        heads = torch.load(self.checkpoint_dir / "heads.pt", map_location="cpu")
        self.category_head.load_state_dict(heads["category_head"])
        self.subcategory_head.load_state_dict(heads["subcategory_head"])
        self.priority_head.load_state_dict(heads["priority_head"])
        self.encoder.to(self.device).eval()
        self.category_head.to(self.device).eval()
        self.subcategory_head.to(self.device).eval()
        self.priority_head.to(self.device).eval()
    ...
    def _forward(self, text):
        enc = self.tokenizer(text, truncation=True, padding=True, max_length=128, return_tensors="pt")
        enc = {k: v.to(self.device) for k, v in enc.items()}
        with torch.no_grad():
            out = self(**enc)
            return out["category_logits"], out["subcategory_logits"], out["priority_pred"]
    ...
    def explain(self, text: str, top_k: int = 8):
        ...
        class_idx = self.taxonomy["categories"].index(prediction["category"])
        explainer = build_transformer_shap_explainer(self, self.tokenizer, self.device)
        return explain_transformer(explainer, text, predicted_class_idx=class_idx, top_k=top_k)
